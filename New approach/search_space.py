...
# Definir una red neuronal simple con UNA neurona
class SimplestNN(nn.Module):
    def __init__(self, input_size, activation_fn):
        super(SimplestNN, self).__init__()
        self.fc = nn.Linear(input_size, 1)  # Una única neurona
        self.activation = activation_fn()   # Activación variable
    
    def forward(self, x):
        return self.activation(self.fc(x))  # Aplicamos activación a la salida
